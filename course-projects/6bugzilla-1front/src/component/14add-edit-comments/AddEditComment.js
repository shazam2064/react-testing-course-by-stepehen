...
function AddEditComment({ comment, bugId, editMode, triggerReload }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditMode) {
                ...
                await createComment(bugId, commentState.text);
            }
            setCommentState(getInitialCommentsState());
            triggerReload();
        } catch (error) {
    ...
    return (
        <div className="mb-3">
            ...
            <Form onSubmit={handleSubmit}>
                ...
                <button type="submit" className="btn btn-outline-secondary">{isEditMode ? 'Update Comment' : 'Add Comment'}</button>
            </Form>
        </div>
    );
}
