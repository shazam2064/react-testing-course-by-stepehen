...
function AddEditUser(props) {
    ...
    useEffect(() => {
        if (isEditMode) {
            fetchAdminUserById(adminUserId).then(adminUser => {
                ...
                setTitle(`Edit User: ${adminUser.name}`);
    ...
    }, [adminUserId, isEditMode, setTitle]);
    ...
    return (
        <section className="container p-5 my-4 col-6 mx-auto bg-light border-3 border rounded">
            ...
            <Form onSubmit={handleSubmit} method="Post">
                ...
                <div className="d-flex align-items-center justify-content-center">
                    <button className="btn btn-outline-secondary mb-3">{isEditMode ? 'Update User' : 'Add User'}</button>
                </div>
            </Form>
        </section>
    );
