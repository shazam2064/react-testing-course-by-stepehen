...
describe('Question Controller', () => {
    ...
    describe('Question Controller - DELETE Question', () => {
        it('should delete a question and return 200 with details', async () => {
            const mockQuestionId = '68ee68e62869fd5ce11a7c78';
            const mockQuestion = {
                _id: mockQuestionId,
                creator: '68ecfe5f977174350fab2a37',
            ...
            jest.spyOn(Question, 'findById').mockResolvedValueOnce(mockQuestion);
            jest.spyOn(Question, 'findByIdAndDelete').mockResolvedValueOnce(mockQuestion);
            jest.spyOn(User, 'findById').mockResolvedValueOnce({
                questions: { pull: jest.fn() },
                save: jest.fn().mockResolvedValueOnce({}),
            });
    ...
    describe('Question Controller - GET Question by Tag', () => {
        it('should return 200 and a list of questions for a tag', async () => {
            ...
            const mockQuestions = [
                {
                    ...
                    voters: [],
                    createdAt: '2025-10-14T15:14:46.960Z',
                    updatedAt: '2025-10-15T15:40:04.582Z',
                    __v: 1
                }
            ];
            ...
            jest.spyOn(Question, 'find').mockImplementationOnce(() => ({
                populate: () => ({
                    populate: () => ({
                        populate: () => Promise.resolve(mockQuestions)
                    })
                })
            }));

            const response = await request(app)
                .get(`/questions/tag/${mockTagId}`)
                .set('Authorization', `Bearer ${validToken}`);
        ...
        it('should return 404 if no questions found for the tag', async () => {
            const mockTagId = '68f63f982eca3e875b58ad7b';
